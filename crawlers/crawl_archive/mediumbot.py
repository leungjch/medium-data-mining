...
class ArticleItem(scrapy.Item):
    ...
    def __repr__(self):
        return repr({"StoryURL": self['StoryURL']})
...
class MediumbotSpider(scrapy.Spider):
    ...
    # get tags
    tags = []
    with open('tags_years_sorted_by_relevance_m27.csv', newline='') as f:
        reader = csv.reader(f)
        tags = list(reader)
        # tags = sum(reader,[])
    ...
    def parse(self, response):
        ...
        if response.status in self.handle_httpstatus_list:
            ...
            if response.status == 429:
                ...
                time.sleep(120) # Pause for 120 seconds to renew rate limit
            ...
            if re.match("^https:\/\/medium\.com\/tag\/[a-z\-]+\/archive$", str_url):
                ...
                # Limit archive from 2015-2020
                filter_years = ['2020','2019','2018','2017','2016','2015']
                list_sub_index = [x for x in list_sub_index if any(y in x for y in filter_years)]
            ...
            # is year now
                ...
                # Apply a filter to months if year is 2015 or 2020
                # August 2015 - April 2020
                month_filter = []

                # Limit beginning from August 2015
                if any("2015/08" in x for x in list_sub_index):
                    month_filter = ['2015/08', '2015/09', '2015/10', '2015/11', '2015/12']

                # Limit to April 2020
                if any("2020/04" in x for x in list_sub_index):
                    month_filter = ['2020/01', '2020/02', '2020/03', '2020/04']

                if len(month_filter) > 0:
                    list_sub_index = [x for x in list_sub_index if any(y in x for y in month_filter)]
    ...
    def parse_end_page(self, response):
        ...
        if response.status in self.handle_httpstatus_list:
            ...
            print('archive url', str_url, 'with ', len(response.css('div.postArticle')), " stories with max", self.max_stories)
    ...
    #     current_index = meta_copy['StoryIndex']
