...
WHERE RootDomain LIKE '%medium.com%' 
   OR RootDomain LIKE '%towardsdatascience.com%'
   OR RootDomain LIKE '%codeburst.io%'
GROUP BY date
ORDER BY date ASC
...
SELECT
  DATE_TRUNC(DATE(CreationDate), MONTH) as date,
  COUNT(*) as count
FROM sotorrent-org.2020_03_15.Comments
INNER JOIN sotorrent-org.2020_03_15.CommentUrl
ON Comments.Id = CommentUrl.CommentId
WHERE RootDomain LIKE '%medium.com%' 
   OR RootDomain LIKE '%towardsdatascience.com%'
   OR RootDomain LIKE '%codeburst.io%'
GROUP BY date
